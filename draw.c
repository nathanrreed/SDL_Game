...
void draw_text(TextBox* textbox) {
    render_text((*textbox->toDraw)(textbox), &textbox->rect, textbox->flags);
}

void draw_button(ButtonElement *btn) {
    if(btn == NULL)
        return;
    ...
    render_text(btn->text, &rect, btn->flags);
}
...
void draw_objects(Array* objs) {
    ...
    for(int i = 0; i < objs->length; i++) {
        ...
        switch (object->type) {
            ...
            // }
            case OBJECT_MENU: {
                if((object->menu.flags & MENU_HIDDEN) > 0) // TODO chcck if this needs to be changed to let input do stuff
                    break;
                ...
                for(int j = 0; j < object->menu.buttons->length; j++) {
                    draw_button(get(object->menu.buttons, j));
                }
